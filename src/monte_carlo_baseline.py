...
PROJECT = {"analysis_type": "monte_carlo", "category": "Stochastic Models & Market Risk", "columns": ["scenario_id", "base_value", "annual_mu", "annual_sigma", "horizon_days", "n_paths", "seed"], "methods": ["로그정규 terminal value를 난수 시드와 함께 생성해 재현성을 확보한다.", "평균, 분위수, 손실확률을 비교해 분포 가정의 영향을 직관적으로 설명한다.", "입력값은 분포 가정의 차이를 비교하도록 구성한 합성 시나리오다."], "module_name": "monte_carlo_baseline", "objective": "확률분포 가정이 금융 변수의 미래 범위와 손실확률에 미치는 영향을 시뮬레이션한다.", "question": "평균과 변동성 가정이 바뀔 때 1년 뒤 포트폴리오 가치 분포는 어떻게 달라지는가?", "rows": [["BASELINE", 100.0, 0.065, 0.18, 252, 4000, 701], ["LOW_VOL", 100.0, 0.045, 0.1, 252, 4000, 702], ["STRESS_VOL", 100.0, 0.025, 0.32, 252, 4000, 703]], "sample_name": "simulation_assumptions.csv", "title_en": "Monte Carlo Simulation", "title_ko": "몬테카를로 시뮬레이션"}
