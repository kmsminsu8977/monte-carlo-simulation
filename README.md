...
몬테카를로 민감도 분석용 표와 질문은 다음 명령으로 재생성합니다.

```bash
python -m src.generate_concept_materials
```

민감도 분석 자료는 `docs/concept_guide.md`에서 읽는 순서와 해석 기준을 확인하고,
`outputs/tables/concept_*.csv` 파일로 직접 점검할 수 있습니다.
...
- `data/sample/concept_scenarios.csv`: 민감도 분석 자료 생성용 합성 민감도 입력값
- `docs/methodology.md`: 계산 절차, 입력/출력 정의, 해석상 주의점
- `docs/concept_guide.md`: 몬테카를로 핵심 수식, 해석 체크리스트, 분석 질문
- `outputs/tables/baseline_results.csv`: 현재 baseline 산출물
- `outputs/tables/concept_distribution_summary.csv`: 민감도 시나리오별 terminal value 분포 요약
- `outputs/tables/concept_convergence_check.csv`: 경로 수별 수렴성 점검표
- `outputs/tables/concept_question_cards.csv`: 핵심 개념 질문 카드
- `outputs/tables/concept_validation_questions.csv`: 검증 질문과 답안
