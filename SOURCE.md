...
이 문서는 `monte-carlo-simulation` 컨텐츠를 만들 때 참고해야 할 로컬 source inventory입니다. 새 코드, 데이터 예시, 문서, 산출물을 만들 때는 이 파일과 `AGENTS.md`를 먼저 확인합니다.

## 프로젝트 식별

- 프로젝트명: 몬테카를로 시뮬레이션 (Monte Carlo Simulation)
- 연구 카테고리: Stochastic Models & Market Risk
- 핵심 질문: 평균과 변동성 가정이 바뀔 때 1년 뒤 포트폴리오 가치 분포는 어떻게 달라지는가?
...
- KAIST-DFMBA/Pension-Fund/Retirement-Investing/monte.ipynb
- KAIST-DFMBA/monte-carlo-pricing-simulation/

## 참고 초점

- 난수 시뮬레이션의 입력 가정과 결과 분포 분리
- 경로 수, seed, 분위수 기반 결과 해석
- Monte Carlo 결과를 표와 차트 산출물로 남기는 구조
